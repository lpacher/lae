...
The top-level RTL module is `CounterBCD_4digit_display.v`. Open with a **text editor** application 
all RTL sources already prepared for you and try to understand the working principle of the proposed
digital design:

```
% gedit rtl/*.v &    (for Linux users)

% n++ rtl\*.v        (for Windows users)
```

<br />

The RTL code of the BCD counter (not shown in the above schematic) has been already discussed
and simulated in `lab7` and can be reviewed at the following link:
...
Despite its simplicity the proposed design provides a first example of an RTL design
with **multiple-levels of hierarchy**. In fact the top-level module instantiates
other sub-modules such as a 7-segment decoder, a 2-bit binary/one-hot decoder and
an N-digit BCD-counter, with the N-digit BCD-counter itself composed of a module-9 counter.

In order to graphically visualize and better understand the overall design organization
you can perform a simple **RTL elaboration** in Vivado that at the end generates a so called
**RTL schematic**. For this purpose run the following command in the terminal:
...
<img src="doc/pictures/BCD_counter_vivado.png" alt="drawing"/>
...
> **QUESTION**
>
> How many levels of design hierarchy can you identify in the design ?
>
...
As you can inspect from the RTL schematic the actual implementation for the `OneHotAnodeDecoder`
module is missing and Vivado infers a **black box** (empty module) for this block.

<br />

<img src="doc/pictures/BlackBox.png" alt="drawing" width="350"/>

<br />

Open with your preferred text-editor the `rtl/OneHotAnodeDecoder.v` template already
prepared for you:

```
% gedit rtl/OneHotAnodeDecoder.v &    (for Linux users)

% n++ rtl\OneHotAnodeDecoder.v        (for Windows users)
```

<br />

To complete the RTL design **try yourself** to code in Verilog the functionality of the
**anode-decoder**, which is a simple 2-bit to 4-bit **binary/one-hot decoder** to drive
7-segment display module anodes:
...
Choose the desired **coding style** to implement the requested functionality
for this block. Feel free to use simple continuous `assign` statements or higher-level
behavioural constructs such as `for` loops, `case` or `if/else` statements.

Please remind that you can check for syntax errors at any time by compiling
your source code with:

```
% make compile hdl=rtl/OneHotAnodeDecoder.v
```
...
Add a **debouncer** to the `button` input in order to **filter out spurious glitches** due to **mechanical chattering**
when pressing the external push-button to increment the counter:
...
Reduce the **refresh frequency** for the 7-segment display module below 0.1 Hz.
Re-run the flows from scratch up to FPGA programming and
verify the functionality of the new firmware after your changes.
