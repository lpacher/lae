...
* capture and debug logic signals in the Vivado _Hardware Manager_
* setup different trigger conditions for ILA debug probes
...
search for "ila" in the _Search_ bar. Right-click on the IP and select **Customize IP**.
...
along with the main bitstream (`.bit`) file. This file is automatically generated for you when
running **Project mode** scripts, while it's up to the user to export this file when working
with **Non Project mode** scripts with the `write_debug_probes` Tcl command.
