...
```
% cd Desktop/lae/fpga/practicum/1_arty_board
```
...
Open the _Hardware Manager_ and establish a new connection through USB/JTAG as discussed before.
...
Replace the functionality of the inverter with a simple **buffer**. For the sake of simplicity simply open and modify
the original `Inverter.v` source file and update the `assign` statement as follows:
...
Save and exit once done. Re-run the flows in batch mode:
...
Capture new digital waveforms at the oscilloscope.
