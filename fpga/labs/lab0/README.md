...
A small **mixed-language HDL design example** is provided to **test your command-line environment setup** and **tools installation**.

The proposed digital system is a parameterized synchronous counter described in VHDL and instantiating also Xilinx-specific FPGA device primitives
such as a Phase-Locked Loop (PLL) clock-management IP block plus some I/O buffers.<br />
The behaviour of the counter is then simulated by using a Verilog testbench module that generates clock, reset and enable control signals.<br />
The code is synthesizable and can be also implemented on real FPGA hardware targeting
...
that will be used in the course. Additional debug features are also part of the implemented design.<br />
A simple GNU `Makefile` is used to **automate all flows** from the command line.
